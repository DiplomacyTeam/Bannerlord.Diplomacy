    ...
    internal sealed class KingdomRulerRepairBehavior : CampaignBehaviorBase
    {
        ...
        public static bool EnsureRuler(Kingdom? kingdom)
        {
            ...
            try
            {
                ...
                var eligible = EligibleRulingClans(kingdom).ToList();

                if (eligible.FirstOrDefault() is { } replacement)
                {
                    ...
                    // Same as the vanilla repair loop in ClanVariablesCampaignBehavior: the clan crowned
                    // above only holds the throne until the kingdom votes, so let it vote.
                    if (eligible.Count > 1 && !kingdom.UnresolvedDecisions.OfType<KingSelectionKingdomDecision>().Any())
        ...
        /// <summary>
        /// Picks the successor vanilla would have picked. <see cref="ChangeClanLeaderAction"/> scores the
        /// candidates with <see cref="Clan.GetHeirApparents"/> and can't be called here, so the scoring is
        /// reused directly; the age ordering is only a fallback for a clan that scores nobody.
        /// </summary>
        private static Hero? FindHeir(Clan clan)
        {
            var heirApparents = clan.GetHeirApparents();

            if (heirApparents.Count > 0)
                return heirApparents.OrderByDescending(h => h.Value).First().Key;
        ...
        private static IEnumerable<Clan> EligibleRulingClans(Kingdom kingdom) => kingdom.Clans
